...
-- =========================================================
-- EMPLOYEE EXCEL ROSTER SYNC 2026-05-11
-- Source: c:\Users\Elhamd\Desktop\بيانات الموظفين.xlsx
-- =========================================================
begin;
...
-- =========================================================
insert into public.database_migration_status (name, status, notes)
values ('084_v31_production_deploy_ready_keep_dev_files', 'APPLIED', 'v31-production-hardening-086')
on conflict (name) do update
  set status = excluded.status,
      notes = excluded.notes,
      applied_at = now();
...
insert into public.database_migration_status (name, status, notes)
values ('085_full_system_audit_fix', 'APPLIED', 'Fixed service-worker Arabic fallback notifications, CSP reverse geocoding, and migration-status marker compatibility')
on conflict (name) do update set status = excluded.status, notes = excluded.notes, applied_at = now();


-- 088) Final audit alignment marker.
create table if not exists public.database_migration_status (
  id text primary key default gen_random_uuid()::text,
  name text unique not null,
  status text not null default 'APPLIED',
...
insert into public.database_migration_status (name, status, notes)
values ('088_final_audit_alignment', 'APPLIED', 'Package v31-production-hardening-088: expectedPatch/cache/version alignment, migration verification alignment, and health diagnostics polish')
on conflict (name) do update set status = excluded.status, notes = excluded.notes, applied_at = now();

insert into public.system_settings (key, value, description)
values ('release_088_final_audit_alignment', jsonb_build_object('version', 'v31-production-hardening-088', 'expectedPatch', '088_final_audit_alignment', 'appliedAt', now()), 'Final audit alignment marker for HR Ahla Shabab package 088')
on conflict (key) do update set value = excluded.value, description = excluded.description, updated_at = now();

-- 089) Codex full deploy alignment marker.
...
insert into public.database_migration_status (name, status, notes)
values ('089_codex_full_deploy_alignment', 'APPLIED', 'Package v31-production-hardening-089: full audit, security fix, version alignment, edge function verification, production deployment.')
on conflict (name) do update set status = excluded.status, notes = excluded.notes, applied_at = now();

insert into public.system_settings (key, value, description)
values ('release_089_codex_full_deploy_alignment', jsonb_build_object('version', 'v31-production-hardening-089', 'expectedPatch', '089_codex_full_deploy_alignment', 'appliedAt', now(), 'previousPatch', '088_final_audit_alignment'), 'Codex full deploy alignment marker for HR Ahla Shabab package 089.')
on conflict (key) do update set value = excluded.value, description = excluded.description, updated_at = now();
