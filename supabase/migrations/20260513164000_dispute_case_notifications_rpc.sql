-- Send dispute notifications from a security-definer RPC so employees do not
-- need direct INSERT access to other users' notification rows.
...
begin
  ...
  select
    ...
    jsonb_build_object(
      'route', 'committee-hub',
      'type', 'DISPUTE_CASE_CREATED',
      'disputeCaseId', v_case.id,
  ...
  if v_case.related_employee_id is not null then
    ...
    select
      ...
      jsonb_build_object(
        'route', 'action-center',
        'type', 'DISPUTE_RELATED_PARTY',
        'disputeCaseId', v_case.id,
        'privacyLevel', 'anonymous_counterparty'
      ),
      now()
    from public.employees e
    left join public.profiles p on p.id = e.user_id or p.employee_id = e.id
